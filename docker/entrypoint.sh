...
init_config() {
    ...
    if [ -n "$GAME_PARAMS" ]; then
      echo "No additional game parameters detected"
    fi
}
...
cd /opt/fsm && ./factorio-server-manager --conf /opt/fsm-data/conf.json --dir /opt/factorio --port 80 ${GAME_PARAMS}